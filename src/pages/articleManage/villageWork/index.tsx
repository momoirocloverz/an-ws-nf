...
import { articleList, articleType, addArticle, editArticle, deletArticle, editArticleStatus } from '@/services/operationCanter';
...
const handleAdd = async (fields: any) => {
  try {
    ...
    const _data = await addArticle({...fields, sub_category_id: '4', category_id: '1'});
    if (_data.code === 0) {
...
const handleUpdate = async (fields: any) => {
  ...
  try {
    delete fields.cover_image;
    const _data = await editArticle({...fields, sub_category_id: '4', category_id: '1'})
    if (_data.code === 0) {
          ...
          <ButtonAuth type="EDIT">
            <a
              onClick={() => {
                ...
                setFormValues({...record, from: 'sanwu'});
              }}
            >
              编辑
            </a>
        ...
        toolBarRender={(action, { selectedRows }) => [
          <ButtonAuth type="CREATE">
            <Button icon={<PlusOutlined />} type="primary" onClick={() => {
              handleModalVisible(true);
              setIsEdit(false);
              setFormValues({from: 'sanwu', parent_category_id: '1', category_id: '4'});
            }}>
              新建文章
            </Button>
          </ButtonAuth>
