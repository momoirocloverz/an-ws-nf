...
const handleAdd = async (fields: any) => {
  try {
    ...
    const cateArr = [...fields['category_id']] || []
    const sub_category_id = cateArr[1]
    const category_id = cateArr[0]
    const _data = await addArticle({...fields, sub_category_id: sub_category_id, category_id: category_id});
    if (_data.code === 0) {
...
const handleUpdate = async (fields: any) => {
  ...
  try {
    ...
    const sub_category_id = cateArr[1]
    const category_id = cateArr[0]
    const _data = await editArticle({...fields, sub_category_id: sub_category_id, category_id: category_id})
    if (_data.code === 0) {
...
const ArticleList: React.FC<any> = (props) => {
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    },
    {
      ...
      width: 130,
      render: (_, record) => {
        return (<p>{record.category_title.indexOf('-')>-1?record.category_title.replace('-',''):record.category_title}</p>)
      },
      renderFormItem: (_, { type, defaultRender, ...rest }, form) => {
        ...
        if (status !== 'open') {
          return <Cascader
            ...
            onChange={changeCate}
            placeholder="请选择分类"
            fieldNames={{label: 'name', value: 'category_id', children: 'sub'}}
          />;
        }
      ...
      render: (_, record) => (
        <>
          ...
          <ButtonAuth type="EDIT">
            <a
              onClick={() => {
                // record.category_id = [record.category_id, record.sub_category_id]
                // console.log(record.category_id, 'id')
                setIsEdit(true);
                handleModalVisible(true);
                setFormValues({...record, from: 'sanwu', type: 'article'});
              }}
            >
              编辑
            </a>
  ...
  const getArticleList = async (val:any) => {
    ...
    valObj['category_id'] = cateArr[valObj.category_id&&valObj.category_id.length>1?valObj.category_id.length - 1:0]
    valObj['flag']='article';
    console.log(valObj)
        ...
        toolBarRender={(action, { selectedRows }) => [
          <ButtonAuth type="CREATE">
            <Button icon={<PlusOutlined />} type="primary" onClick={() => {
              handleModalVisible(true);
              setIsEdit(false);
              setFormValues({from: 'sanwu', type: 'article'});
            }}>
              新建文章
            </Button>
          </ButtonAuth>
