...
const TableList: React.FC<any> = (props) => {
  ...
  const [ length, getArrLength ] = useState<any>(0)
      ...
      if (info.file.status === 'done') {
        if(info.file.response.code === 0) {
          ...
          if(info.file.response.data.length > 0) {
            Modal.error({
              content: `手机号为${info.file.response.data.join(",")}导入失败`,
            });
          } else {
        ...
        toolBarRender={(action, { selectedRows }) => [
          <ButtonAuth type="UPLOAD_SHARE_MODEL">
            <Button type="primary" onClick={() => {
              window.location.href = '/股金分红导入模板.xlsx'
            }}>
              股金分红模板
            </Button>
          </ButtonAuth>,
