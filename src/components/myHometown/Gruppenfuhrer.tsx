...
import {
  deleteGruppenfuhrer, getGruppenfuhrers,
} from '@/services/myHometown';
import ButtonAuth from '@/components/ButtonAuth';
import { downloadAs } from '@/pages/agricultureSubsidies/utils';
import GruppenfuhrerModal from '@/components/myHometown/GruppenfuhrerModal';
...
export default function Gruppenfuhrer({
  ...
  const loadData = async (rawParams) => {
    ...
    const result = await getGruppenfuhrers(params);
          ...
          <Button
            ...
            onClick={() => {
              Modal.confirm({
                ...
                onOk: async () => {
                  try {
                    await deleteGruppenfuhrer(record.id);
        ...
        toolBarRender={() => [
          <ButtonAuth type="IMPORT">
            <Button
              type="primary"
              onClick={() => {
                window.location.href = 'https://wsnbh-img.hzanchu.com/acfile/3ed9f430a4dbdeced870fbc22692a9eb.xlsx';
              }}
            >
              下载模板
            </Button>
          ...
          <ButtonAuth type="IMPORT">
            <ImportBtn api="import_team_leader_grid_overview" onSuccess={() => tableRef.current?.reload()} />
          </ButtonAuth>,
          <ButtonAuth type="EXPORT">
            <Button
              ...
              onClick={() => {
                ...
                getGruppenfuhrers({ ...exportParams, asFile: 1 }).then((result) => {
                  downloadAs(result, `${new Date().toLocaleString()}组长网格概况导出记录.xls`, 'application/vnd.ms-excel');
                }).catch((e) => {
